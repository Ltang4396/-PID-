...
public:
        ...
        ref_longitude_(0.0), ref_latitude_(0.0)
    {
        ...
        pub_gnss_relative_ = nh_.advertise<nav_msgs::Odometry>("gnss_relative", 10);
        pose_angle_pub_ = nh_.advertise<nav_msgs::Odometry>("pose_angle", 10);
        pose_velocity_pub = nh.advertise<geometry_msgs::Pose2D>("pose_velocity", 1000);  // 创建GPS数据发布器
    ...
    void run()
    {
        ...
        while (ros::ok())
        {
            ...
            if (pose_xy_received_ && pose_LLA_received_)
            {
                ...
                odom_msg.header.frame_id = "map";  // 或 "odom" / "gnss_link" 等根据实际坐标系
                // 位置信息
                odom_msg.pose.pose.position.x = x_rel;
                odom_msg.pose.pose.position.y = y_rel;
                pub_gnss_relative_.publish(odom_msg);
...
private:
    ...
    void processGPCHC(const std::string& frame)
    {
        ...
        try
        {
            ...
            nav_msgs::Odometry pose_angle;
            pose_angle.header.stamp = ros::Time::now();
            pose_angle.pose.pose.position.x = heading;
            pose_angle.pose.pose.position.y = pitch;
            pose_angle.pose.pose.position.z = roll;
            ...
            geometry_msgs::Pose2D pose_vel;
            pose_vel.x = ve; //东西速度
            pose_vel.y = vn;//北向速度
            pose_vel.theta = v;//车辆速度
            pose_velocity_pub.publish(pose_vel);
